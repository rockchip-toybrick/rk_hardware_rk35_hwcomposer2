# Security Policy
## Supported Versions
The following versions of drmhwc2 are currently supported with security updates:
| Version                      | Supported                      |
| :--------------------------- | :----------------------------- |
...
If you discover a security vulnerability in drmhwc2, please report it privately using the following methods:
### Reporting Channels
1. **Primary Contact** (Recommended)
   - Email the maintainer at: **toybrick@rock-chips.com**
   - Subject format: `[SECURITY] drmhwc2 vulnerability report - [brief description]`
...
- **Affected Versions**: Specific drmhwc2 version or Git commit hash
- **Affected Components**: Specific modules (e.g., Mali_driver/RGA_driver/dma-buf/IOMMU, etc.)
- **Environment Details**:
...
When using drmhwc2, the following security measures are recommended:
1. **Permission Control**: Restrict access to `/dev/dri/*` device nodes with appropriate permissions
2. **Timely Updates**: Keep drmhwc2 and the drm kernel driver updated to the latest version to receive the latest security fixes
...
View [GitHub Security Advisories](https://github.com/rockchip-toybrick/rk_hardware_rk35_hwcomposer2/security/advisories) for publicly disclosed security announcements.
---
Thank you for contributing to drmhwc2 security!