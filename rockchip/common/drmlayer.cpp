...
int DrmHwcBuffer::SetBoInfo(uint64_t buffer_id,
                            uint32_t fd, uint32_t width,
...
int DrmHwcLayer::ImportBuffer(Importer *importer) {
  ...
  // 另外：RK3399 NV12_10场景不能用CopyBufferHandle，gralloc会返回错误
...
void DrmHwcLayer::ResetInfoFromStore(){
  ...
  Init();
  HWC2_ALOGD_IF_DEBUG(
             "reset:DrmHwcLayer[%4u] Buffer[w/h/s/hs/format]=[%4d,%4d,%4d,%4d,%4d] Fourcc=%c%c%c%c Transform=%-8.8s(0x%x) Blend[a=%d]=%-8.8s "
             "source_crop[l,t,r,b]=[%5.0f,%5.0f,%5.0f,%5.0f] display_frame[l,t,r,b]=[%4d,%4d,%4d,%4d],skip=%d,afbcd=%d,gles=%d\n",
             uId_,iWidth_,iHeight_,iStride_,iHeightStride_,iFormat_,uFourccFormat_,uFourccFormat_>>8,uFourccFormat_>>16,uFourccFormat_>>24,
             TransformToString(transform).c_str(),transform,alpha,BlendingToString(blending).c_str(),
             source_crop.left,source_crop.top,source_crop.right,source_crop.bottom,
             display_frame.left,display_frame.top,display_frame.right,display_frame.bottom,bSkipLayer_,bAfbcd_,bGlesCompose_);
...
int DrmHwcLayer::DumpInfo(String8 &out){
    if(bFbTarget_)
      ...
      out.appendFormat( "DrmHwcLayer[%4u] Buffer[w/h/s/bs/hs/format]=[%4d,%4d,%4d,%4d,%4d,%4d] Fourcc=%c%c%c%c Transform=%-8.8s(0x%x) Blend[a=%d]=%-8.8s "
                        "source_crop[l,t,r,b]=[%5.0f,%5.0f,%5.0f,%5.0f] display_frame[l,t,r,b]=[%4d,%4d,%4d,%4d],skip=%d,afbcd=%d,rfbcd=%d hdr=%d fps=%f \n",
                       uId_,iWidth_,iHeight_,iStride_,iHeightStride_,iByteStride_,iFormat_,uFourccFormat_,uFourccFormat_>>8,uFourccFormat_>>16,uFourccFormat_>>24,
                       TransformToString(transform).c_str(),transform,alpha,BlendingToString(blending).c_str(),
                       source_crop.left,source_crop.top,source_crop.right,source_crop.bottom,
                       display_frame.left,display_frame.top,display_frame.right,display_frame.bottom,bSkipLayer_,bAfbcd_,bRfbcd_,
                       bHdr_, fFps_);
    return 0;
}
